...
// 😂😂😂😂
case 'menu': {
    // FIX: define reply helper locally so catch block won't crash
    const reply = (text) => socket.sendMessage(sender, { text }, { quoted: msg }).catch(() => {});
    ...
    try {
        ...
        // FIX: Send video note separately, wrapped in its own try/catch
        // so if it fails, the main menu message still sends
        try {
            await socket.sendMessage(sender, {
            ...
            console.log("⚠️ Video intro failed (non-fatal):", videoErr?.message || videoErr);
        }
...
> *〠 𝐏𝙾𝚆𝙴𝚁𝙴𝙳 𝗕𝗬 𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 𝐕.4 🥷🇱🇰*`;
        ...
        // --- FIX: Use listMessage (interactive list) instead of deprecated buttons on document ---
        // WhatsApp no longer delivers button/document hybrid messages reliably.
        // listMessage works correctly on all current WhatsApp versions.
        const sections = [
            {
                title: "𝙰𝚂𝙷𝙸𝚈𝙰-𝙼𝙳 ᴍᴇɴᴜ ʟɪꜱᴛ 🙌",
                rows: [
                    { title: "❄ ᴅᴏᴡɴʟᴏᴀᴅ ᴄᴍᴅ",       description: "𝙰𝚂𝙷𝙸𝚈𝙰-𝙼𝙳 ᴠ.4.0.0 ᴅᴏᴡɴʟᴏᴀᴅ ᴍᴇɴᴜ 🍷",      rowId: `${config.PREFIX}download` },
                    { title: "❄ ᴀʟɪᴠᴇ ᴄᴍᴅ",           description: "𝙰𝚂𝙷𝙸𝚈𝙰-𝙼𝙳 ᴠ.4.0.0 ᴀʟɪᴠᴇ ᴍᴇɴᴜ 🍷",        rowId: `${config.PREFIX}alive` },
                    { title: "❄ ᴀɪ ᴄᴏᴘᴀɴɪᴏɴ ᴄᴍᴅ",     description: "𝙰𝚂𝙷𝙸𝚈𝙰-𝙼𝙳 ᴠ.4.0.0 ᴀɪ ᴄᴏᴘᴀɴɪᴏɴ ᴍᴇɴᴜ 🍷",  rowId: `${config.PREFIX}ai` },
                    { title: "❄ ꜱᴍᴀʀᴛ ꜱᴇᴀʀᴄʜ ᴄᴍᴅ",    description: "𝙰𝚂𝙷𝙸𝚈𝙰-𝙼𝙳 ᴠ.4.0.0 ꜱᴇᴀʀᴄʜ ᴍᴇɴᴜ 🍷",       rowId: `${config.PREFIX}search` },
                    { title: "❄ ᴏᴡɴᴇʀ ɪɴꜰᴏ",           description: "𝙰𝚂𝙷𝙸𝚈𝙰-𝙼𝙳 ᴠ.4.0.0 ᴏᴡɴᴇʀ ɪɴꜰᴏ 🍷",       rowId: `${config.PREFIX}owner` },
                    { title: "❄ ꜱʏꜱᴛᴇᴍ ꜱᴛᴀᴛᴜꜱ",        description: "𝙰𝚂𝙷𝙸𝚈𝙰-𝙼𝙳 ᴠ.4.0.0 ꜱʏꜱᴛᴇᴍ ꜱᴛᴀᴛᴜꜱ 🍷",    rowId: `${config.PREFIX}ping` },
                    { title: "❄ ᴛᴏᴏʟ / ᴄʀᴇᴀᴛɪᴠᴇ",      description: "𝙰𝚂𝙷𝙸𝚈𝙰-𝙼𝙳 ᴠ.4.0.0 ᴛᴏᴏʟ ᴍᴇɴᴜ 🍷",       rowId: `${config.PREFIX}tool` }
                ]
            }
        ];

        // FIX: Send image + listMessage — this works on all current WhatsApp versions
        await socket.sendMessage(sender, {
            image: { url: MENU_IMG },
            caption: caption,
            footer: `> *〠 𝐏𝙾𝚆𝙴𝚁𝙴𝙳 𝗕𝗬 𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 𝐕.4 🥷🇱🇰*`,
        ...
        // Send list menu as a separate message (listMessage is reliable on all WA versions)
        await socket.sendMessage(sender, {
            text: `*🥷 𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 ᴠ.4 — ꜱᴇʟᴇᴄᴛ ᴀ ᴄᴀᴛᴇɢᴏʀʏ*\n\n💬 *${randomQuote}*`,
            footer: `〠 𝐏𝙾𝚆𝙴𝚁𝙴𝙳 𝗕𝗬 𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 𝐕.4 🥷🇱🇰`,
            listMessage: {
                buttonText: "📂 𝐎𝐏𝐄𝐍 𝐌𝐄𝐍𝐔",
                description: `${BOT_NAME} ᴍᴇɴᴜ`,
                listType: 1,
                sections: sections
            }
        }, { quoted: msg });
        ...
        reply("⚠️ Menu load කරන්න බැරි වුණා. නැවත try කරන්න.");
    }
    break;
}
// ==================== DOWNLOAD MENU ====================
