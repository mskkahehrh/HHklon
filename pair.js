...
case 'down_1080': {
    try {
        ...
        if (!json.status || !json.download?.link) return await socket.sendMessage(sender, { text: '*📛 මට කිසිවක් සොයාගත නොහැකි විය :(*' }, { quoted: msg });
...
case 'down_1080d': {
    try {
        ...
        let resizedThumb;
        if (json.thumbnail) {
            const imgRes = await axios.get(json.thumbnail, { responseType: 'arraybuffer' });
