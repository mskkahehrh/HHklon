...
case 'menu': {
  try {
    ...
    const cpuUsageStr = `${cpuLoad} load avg (${cpus.length} cores)`;

    // Respond Speed
    const respondSpeed = msg.messageTimestamp
      ? `${Date.now() - msg.messageTimestamp * 1000}ms`
      : 'N/A';

    // TIME & DATE (Sri Lanka timezone)
    const nowSL = moment().tz('Asia/Colombo');
    const menuTime = nowSL.format('HH:mm:ss');
