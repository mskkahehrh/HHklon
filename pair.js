...
case 'ytmp3': {
    try {
        ...
        // --- Helper: YouTube MP3 via sadaslk-dlcore ---
        ...
        // 4. YouTube Search using ytsearch-venom
        let videoInfo;
        try {
            const searchRes = await yts(text);
            videoInfo = searchRes && searchRes.videos ? searchRes.videos[0] : null;
        } catch (e) {
        ...
        const captionMsg = `*┎━━━━━━━━━━━━━━━━❖●►*\n` +
            ...
            `*┃➤ ⏱ Duration :* ${videoInfo.timestamp || videoInfo.duration}\n` +
        ...
        // 6. Buttons
        const buttons = [
            { buttonId: 'song_audio', buttonText: { displayText: '🎶 Audio Format' }, type: 1 },
            { buttonId: 'song_doc',   buttonText: { displayText: '📂 Document Format' }, type: 1 },
            { buttonId: 'song_ptt',   buttonText: { displayText: '🎤 Voice Format' }, type: 1 }
        ];
        ...
        const sentMessage = await socket.sendMessage(sender, {
            ...
            caption: captionMsg,
            footer: footer || `〠 𝐏𝙾𝚆𝙴𝚁𝙴𝙳 𝗕𝗬 𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 𝐕.4 🥷🇱🇰`,
            buttons: buttons,
            headerType: 4
        }, { quoted: msg });
        ...
        // 8. Listener for button/reply selection
        const handleSongSelection = async ({ messages: replyMessages }) => {
            const replyMek = replyMessages[0];
            if (!replyMek?.message) return;
            if (replyMek.key.remoteJid !== sender) return;
            if (replyMek.key.fromMe) return;

            // Extract selected ID from all known button response types
            let selectedId =
                replyMek.message.buttonsResponseMessage?.selectedButtonId ||
                replyMek.message.buttonsResponseMessage?.selectedDisplayText ||
                replyMek.message.templateButtonReplyMessage?.selectedId ||
                replyMek.message.templateButtonReplyMessage?.selectedDisplayText ||
                replyMek.message.conversation ||
                replyMek.message.extendedTextMessage?.text;

            // Native flow JSON parse (newer WA versions)
            if (!selectedId) {
                try {
                    const paramsJson = replyMek.message.interactiveResponseMessage?.nativeFlowResponseMessage?.paramsJson;
                    if (paramsJson) {
                        const params = JSON.parse(paramsJson);
                        selectedId = params.id || params.display_text;
                    }
                } catch (e) { /* ignore */ }
            }
            ...
            // Check if this reply is linked to our song message (stanzaId) OR sender replied with text
            const ctxId =
                replyMek.message.buttonsResponseMessage?.contextInfo?.stanzaId ||
                replyMek.message.templateButtonReplyMessage?.contextInfo?.stanzaId ||
                replyMek.message.extendedTextMessage?.contextInfo?.stanzaId ||
                replyMek.message.interactiveResponseMessage?.contextInfo?.stanzaId;

            const isReplyToMsg = ctxId === messageID;
            if (!isReplyToMsg) return;

            // Match by button ID, number, or display text (newer WA sends display text)
            let type = null;
            const sid = selectedId.toLowerCase().trim();
            if (sid === 'song_audio' || sid === '1' || sid.includes('audio format')) type = 'audio';
            else if (sid === 'song_doc' || sid === '2' || sid.includes('document format')) type = 'doc';
            else if (sid === 'song_ptt' || sid === '3' || sid.includes('voice format')) type = 'ptt';

            if (!type) return;

            // Stop listening
            socket.ev.removeListener('messages.upsert', handleSongSelection);

            await socket.sendMessage(sender, { react: { text: '⬇️', key: replyMek.key } });

            try {
                // Download MP3 via sadaslk-dlcore
                const mp3Url = await getYouTubeMp3(videoInfo.url);

                if (type === 'ptt') {
                    // Download + convert to OGG voice note
                    const mp3Res = await axios.get(mp3Url, { responseType: 'arraybuffer' });
                    const tempMp3 = require('path').join(require('os').tmpdir(), `song_${Date.now()}.mp3`);
                    const tempOgg = require('path').join(require('os').tmpdir(), `song_${Date.now()}.ogg`);
                    const fsM = require('fs-extra');

                    fsM.writeFileSync(tempMp3, Buffer.from(mp3Res.data));
                    ...
                    await socket.sendMessage(sender, {
                        audio: fsM.readFileSync(tempOgg),
                        mimetype: 'audio/ogg; codecs=opus',
                        ptt: true
                    }, { quoted: replyMek });

                    try { fsM.unlinkSync(tempMp3); } catch(e) {}
                    try { fsM.unlinkSync(tempOgg); } catch(e) {}

                } else if (type === 'doc') {
                    // Document format with thumbnail
                    let resizedThumb;
                    try {
                        const sharp = require('sharp');
                        const thumbRes = await axios.get(videoInfo.thumbnail, { responseType: 'arraybuffer' });
                        resizedThumb = await sharp(Buffer.from(thumbRes.data)).resize(200, 200).toBuffer();
                    } catch(e) { resizedThumb = undefined; }
