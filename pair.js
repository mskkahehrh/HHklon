...
case 'ytmp3': {
    try {
        ...
        let text = (args.join(' ') || '').trim();
        if (!text) {
            return await socket.sendMessage(sender, { text: "❌ *Song name එකක් දෙන්න!*\n\nExample: .song Sanda Eliya" }, { quoted: msg });
        }
        ...
        if (!result) return await socket.sendMessage(sender, { text: "❌ *Song සොයාගන්න බැරි වුණා :(*" }, { quoted: msg });
        ...
        await socket.sendMessage(sender, { text: '❌ Song command error. Try again.' }, { quoted: msg });
    }
    break;
}
case 'ytaa': {
    try {
        ...
        if (!q) return await socket.sendMessage(sender, { text: "❌ *URL එකක් නෑ!*" }, { quoted: msg });

        await socket.sendMessage(sender, { react: { text: '⬇️', key: msg.key } });
        const mp3 = await dlcore.ytmp3(q);
        if (!mp3 || !mp3.url) throw new Error("No download URL");
        ...
        await socket.sendMessage(sender, { text: '❌ Song download error.' }, { quoted: msg });
    }
    break;
}
case 'ytaap': {
    try {
        ...
        const mp3Data = await dlcore.ytmp3(q);
        if (!mp3Data || !mp3Data.url) throw new Error("No download URL");

        const mp3File = path.join(os.tmpdir(), \`ytaap_\${Date.now()}.mp3\`);
        const oggFile = path.join(os.tmpdir(), \`ytaap_\${Date.now()}.ogg\`);

        const dl = await axios.get(mp3Data.url, { responseType: 'arraybuffer' });
        ...
        await socket.sendMessage(sender, { text: '❌ Voice note error.' }, { quoted: msg });
    }
    break;
}
case 'ytad': {
    try {
        ...
        const mp3Data = await dlcore.ytmp3(url);
        if (!mp3Data || !mp3Data.url) throw new Error("No download URL");

        await socket.sendMessage(sender, {
            document: { url: mp3Data.url },
            mimetype: 'audio/mpeg',
            caption: \``${title}`\n\n${config.BOT_FOOTER}\`,
            fileName: \`\${title}.mp3\`
        }, { quoted: msg });
        ...
        await socket.sendMessage(sender, { text: '❌ Document download error.' }, { quoted: msg });
    }
    break;
}
case 'system': {
