...
function setupCommandHandlers(socket, number) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    // ---- BOT HEADER CARD (ping shala style - shown on every bot reply) ----
    const botHeaderCard = {
      key: {
    ...
    if (!command) {
      ...
      if (moviePendingSearch[sender] && !isNaN(replyNum) && replyNum > 0 && replyNum <= moviePendingSearch[sender].results.length) {
        try {
          ...
          if (metadata.thumbnail) {
            await socket.sendMessage(from, { image: { url: metadata.thumbnail }, caption: infoMsg }, { quoted: botHeaderCard });
          } else {
            await socket.sendMessage(from, { text: infoMsg }, { quoted: botHeaderCard });
          }
          const downloadLinks = await getPixeldrainLinks(selected.movieUrl);
          if (!downloadLinks.length) {
            await socket.sendMessage(from, { text: "*❌ No download links found (<2GB)!*" }, { quoted: botHeaderCard });
            return;
          }
          ...
          await socket.sendMessage(from, { text: qualityMsg }, { quoted: botHeaderCard });
        } catch(e) { await socket.sendMessage(from, { text: `*❌ Error:* ${e.message}` }, { quoted: botHeaderCard }); }
        return;
      }
      ...
      if (moviePendingQuality[sender] && !isNaN(replyNum) && replyNum > 0 && replyNum <= moviePendingQuality[sender].movie.downloadLinks.length) {
        try {
          ...
          }, { quoted: botHeaderCard });
        } catch(e) { await socket.sendMessage(from, { text: `*❌ Failed to send movie:* ${e.message}` }, { quoted: botHeaderCard }); }
        return;
      }
...
END:VCARD`
    ...
    try {
        await socket.sendMessage(sender, { text: `🔎 Searching TikTok for: ${query}...` }, { quoted: botHeaderCard });
        ...
        if (!videos || videos.length === 0) {
            return await socket.sendMessage(sender, { text: '⚠️ No videos found.' }, { quoted: botHeaderCard });
        }
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            await socket.sendMessage(sender, { text: `*⏳ Downloading:* ${v.title || 'No Title'}` }, { quoted: botHeaderCard });
        ...
        await socket.sendMessage(sender, { text: `❌ Error: ${err.message}` }, { quoted: botHeaderCard });
    }
...
case 'mp4': {
    try {
        ...
        if (!q) return await socket.sendMessage(sender, { text: '*📛 කරුණාකර වචන කිහිපයක් ලියන්න*' }, { quoted: botHeaderCard });
        ...
        if (!searchResults.videos.length) return await socket.sendMessage(sender, { text: '*📛 මට කිසිවක් සොයාගත නොහැකි විය :(*' }, { quoted: botHeaderCard });
        ...
        await socket.sendMessage(sender, { text: '*🚩 Youtube Error*' }, { quoted: botHeaderCard });
    }
    break;
}
...
case 'down_1080': {
    try {
        ...
        const q = args.join(' ').trim();
        if (!q) return await socket.sendMessage(sender, { text: '*📛 කරුණාකර url එකක් ලබා දෙන්න*' }, { quoted: botHeaderCard });
        ...
        await socket.sendMessage(sender, { text: '*📛 Video Error*' }, { quoted: botHeaderCard });
    }
    break;
}
case 'down_360d':
...
case 'wtype': {
  ...
  try {
    ...
    if (senderNum !== sanitized && senderNum !== ownerNum) {
      ...
      return await socket.sendMessage(sender, { text: '❌ Permission denied. Only the session owner or bot owner can change work type.' }, { quoted: botHeaderCard });
    }
    ...
    await socket.sendMessage(sender, { text: "*❌ Error updating your work type!*" }, { quoted: botHeaderCard });
  }
  break;
}
...
case 'botpresence': {
  ...
  try {
    ...
    if (senderNum !== sanitized && senderNum !== ownerNum) {
      ...
      return await socket.sendMessage(sender, { text: '❌ Permission denied. Only the session owner or bot owner can change bot presence.' }, { quoted: botHeaderCard });
    }
    ...
    if (settings[q]) {
      ...
      await socket.sendMessage(sender, { text: `✅ *Your Bot Presence updated to: ${q}*` }, { quoted: botHeaderCard });
    } else {
      const shonux = {
        key: { remoteJid: "status@broadcast", participant: "0@s.whatsapp.net", fromMe: false, id: "META_AI_PRESENCE3" },
        message: { contactMessage: { displayName: BOT_NAME_FANCY, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${BOT_NAME_FANCY};;;;\nFN:${BOT_NAME_FANCY}\nORG:Meta Platforms\nTEL;type=CELL;type=VOICE;waid=13135550002:+1 313 555 0002\nEND:VCARD` } }
      };
      await socket.sendMessage(sender, { text: "❌ *Invalid option!*\n\nAvailable options:\n- online\n- offline" }, { quoted: botHeaderCard });
    }
  } catch (e) {
    ...
    await socket.sendMessage(sender, { text: "*❌ Error updating your bot presence!*" }, { quoted: botHeaderCard });
  }
  break;
}
...
case 'autotyping': {
  ...
  try {
    ...
    if (senderNum !== sanitized && senderNum !== ownerNum) {
      ...
      return await socket.sendMessage(sender, { text: '❌ Permission denied. Only the session owner or bot owner can change auto typing.' }, { quoted: botHeaderCard });
    }
    ...
    if (settings[q]) {
      ...
      await socket.sendMessage(sender, { text: `✅ *Auto Typing ${q === 'on' ? 'ENABLED' : 'DISABLED'}*` }, { quoted: botHeaderCard });
    } else {
      const shonux = {
        key: { remoteJid: "status@broadcast", participant: "0@s.whatsapp.net", fromMe: false, id: "META_AI_TYPING3" },
        message: { contactMessage: { displayName: BOT_NAME_FANCY, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${BOT_NAME_FANCY};;;;\nFN:${BOT_NAME_FANCY}\nORG:Meta Platforms\nTEL;type=CELL;type=VOICE;waid=13135550002:+1 313 555 0002\nEND:VCARD` } }
      };
      await socket.sendMessage(sender, { text: "❌ *Options:* on / off" }, { quoted: botHeaderCard });
    }
  } catch (e) {
    ...
    await socket.sendMessage(sender, { text: "*❌ Error updating auto typing!*" }, { quoted: botHeaderCard });
  }
  break;
}
...
case 'rstatus': {
  ...
  try {
    ...
    if (senderNum !== sanitized && senderNum !== ownerNum) {
      ...
      return await socket.sendMessage(sender, { text: '❌ Permission denied. Only the session owner or bot owner can change status seen setting.' }, { quoted: botHeaderCard });
    }
    ...
    if (settings[q]) {
      ...
      await socket.sendMessage(sender, { text: `✅ *Your Auto Status Seen ${q === 'on' ? 'ENABLED' : 'DISABLED'}*` }, { quoted: botHeaderCard });
    } else {
      const shonux = {
        key: { remoteJid: "status@broadcast", participant: "0@s.whatsapp.net", fromMe: false, id: "META_AI_RSTATUS3" },
        message: { contactMessage: { displayName: BOT_NAME_FANCY, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${BOT_NAME_FANCY};;;;\nFN:${BOT_NAME_FANCY}\nORG:Meta Platforms\nTEL;type=CELL;type=VOICE;waid=13135550002:+1 313 555 0002\nEND:VCARD` } }
      };
      await socket.sendMessage(sender, { text: "❌ *Invalid option!*\n\nAvailable options:\n- on\n- off" }, { quoted: botHeaderCard });
    }
  } catch (e) {
    ...
    await socket.sendMessage(sender, { text: "*❌ Error updating your status seen setting!*" }, { quoted: botHeaderCard });
  }
  break;
}
...
case 'creject': {
  ...
  try {
    ...
    if (senderNum !== sanitized && senderNum !== ownerNum) {
      ...
      return await socket.sendMessage(sender, { text: '❌ Permission denied. Only the session owner or bot owner can change call reject setting.' }, { quoted: botHeaderCard });
    }
    ...
    if (settings[q]) {
      ...
      await socket.sendMessage(sender, { text: `✅ *Your Auto Call Reject ${q === 'on' ? 'ENABLED' : 'DISABLED'}*` }, { quoted: botHeaderCard });
    } else {
      const shonux = {
    ...
    await socket.sendMessage(sender, { text: "*❌ Error updating your call reject setting!*" }, { quoted: botHeaderCard });
  }
  break;
}
...
case 'arm': {
  ...
  try {
    ...
    if (senderNum !== sanitized && senderNum !== ownerNum) {
      ...
      return await socket.sendMessage(sender, { text: '❌ Permission denied. Only the session owner or bot owner can change status react setting.' }, { quoted: botHeaderCard });
    }
    ...
    if (settings[q]) {
      ...
      await socket.sendMessage(sender, { text: `✅ *Your Auto Status React ${q === 'on' ? 'ENABLED' : 'DISABLED'}*` }, { quoted: botHeaderCard });
    } else {
      const shonux = {
    ...
    await socket.sendMessage(sender, { text: "*❌ Error updating your status react setting!*" }, { quoted: botHeaderCard });
  }
  break;
}
...
case 'mread': {
  ...
  try {
    ...
    if (senderNum !== sanitized && senderNum !== ownerNum) {
      ...
      return await socket.sendMessage(sender, { text: '❌ Permission denied. Only the session owner or bot owner can change message read setting.' }, { quoted: botHeaderCard });
    }
    ...
    if (settings[q]) {
      ...
      await socket.sendMessage(sender, { text: `✅ *Your Auto Message Read: ${statusText}*` }, { quoted: botHeaderCard });
    } else {
      const shonux = {
    ...
    await socket.sendMessage(sender, { text: "*❌ Error updating your message read setting!*" }, { quoted: botHeaderCard });
  }
  break;
}
...
case 'autorecording': {
  ...
  try {
    ...
    if (senderNum !== sanitized && senderNum !== ownerNum) {
      ...
      return await socket.sendMessage(sender, { text: '❌ Permission denied. Only the session owner or bot owner can change auto recording.' }, { quoted: botHeaderCard });
    }
    ...
    if (q === 'on' || q === 'off') {
      ...
      await socket.sendMessage(sender, { text: `✅ *Auto Recording ${q === 'on' ? 'ENABLED' : 'DISABLED'}*` }, { quoted: botHeaderCard });
    } else {
      const shonux = {
        key: { remoteJid: "status@broadcast", participant: "0@s.whatsapp.net", fromMe: false, id: "META_AI_RECORDING3" },
        message: { contactMessage: { displayName: BOT_NAME_FANCY, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${BOT_NAME_FANCY};;;;\nFN:${BOT_NAME_FANCY}\nORG:Meta Platforms\nTEL;type=CELL;type=VOICE;waid=13135550002:+1 313 555 0002\nEND:VCARD` } }
      };
      await socket.sendMessage(sender, { text: "❌ *Invalid! Use:* .autorecording on/off" }, { quoted: botHeaderCard });
    }
  } catch (e) {
    ...
    await socket.sendMessage(sender, { text: "*❌ Error updating auto recording!*" }, { quoted: botHeaderCard });
  }
  break;
}
...
case 'prefix': {
  ...
  try {
    ...
    if (senderNum !== sanitized && senderNum !== ownerNum) {
      ...
      return await socket.sendMessage(sender, { text: '❌ Permission denied. Only the session owner or bot owner can change prefix.' }, { quoted: botHeaderCard });
    }
    ...
    await socket.sendMessage(sender, { text: `✅ *Your Prefix updated to: ${newPrefix}*` }, { quoted: botHeaderCard });
  } catch (e) {
    ...
    await socket.sendMessage(sender, { text: "*❌ Error updating your prefix!*" }, { quoted: botHeaderCard });
  }
  break;
}
//✅✅✅
case 'settings': {
  ...
  try {
    ...
    if (senderNum !== sanitized && senderNum !== ownerNum) {
      ...
      return await socket.sendMessage(sender, { text: '❌ Permission denied. Only the session owner or bot owner can view settings.' }, { quoted: botHeaderCard });
    }
...
> _*〠 𝐏𝙾𝚆𝙴𝚁𝙴𝙳 𝗕𝗬 𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 𝐕.3 🥷🇱🇰*_`; 
    ...
    await socket.sendMessage(sender, { text: "*❌ Error loading settings!*" }, { quoted: botHeaderCard });
  }
  break;
}
...
case 'checkjid': {
  try {
    ...
    if (senderNum !== sanitized && senderNum !== ownerNum) {
      ...
      return await socket.sendMessage(sender, { text: '❌ Permission denied. Only the session owner or bot owner can use this command.' }, { quoted: botHeaderCard });
    }
    ...
    await socket.sendMessage(sender, { text: "*❌ Error checking JID information!*" }, { quoted: botHeaderCard });
  }
  break;
}
...
case 'emojis': {
  ...
  try {
    ...
    if (senderNum !== sanitized && senderNum !== ownerNum) {
      ...
      return await socket.sendMessage(sender, { text: '❌ Permission denied. Only the session owner or bot owner can change status reaction emojis.' }, { quoted: botHeaderCard });
    }
    ...
    await socket.sendMessage(sender, { text: "*❌ Error updating your status reaction emojis!*" }, { quoted: botHeaderCard });
  }
  break;
}
...
╚═══════════════════❒`
        ...
        await socket.sendMessage(sender, { text: pairCode }, { quoted: botHeaderCard });
...
case 'pp': {
  try {
    ...
    if (!data?.results || data.results.length === 0) {
      return socket.sendMessage(sender, { text: '❎ No results found for that query!' }, { quoted: botHeaderCard });
    }
    ...
    if (filtered.length === 0) {
      return socket.sendMessage(sender, { text: '❎ No relevant pastpaper results found.' }, { quoted: botHeaderCard });
    }
        ...
        if (isReply && ['1','2','3','4','5'].includes(text)) {
          ...
          try {
            ...
            if (!dlData?.found || !dlData.pdfs || dlData.pdfs.length === 0) {
              await socket.sendMessage(sender, { react: { text: '❌', key: m.key } });
              await socket.sendMessage(sender, { text: '❎ No direct PDF found for that page.' }, { quoted: botHeaderCard });
              // cleanup
            ...
            await socket.sendMessage(sender, { react: { text: '❌', key: m.key } });
            await socket.sendMessage(sender, { text: `❌ Error fetching PDF: ${err.message}` }, { quoted: botHeaderCard });
            socket.ev.off('messages.upsert', listener);
          }
        }
      } catch (err) {
...
case 'deleteme': {
  ...
  if (senderNum !== sanitized && senderNum !== ownerNum) {
    await socket.sendMessage(sender, { text: '❌ Permission denied. Only the session owner or the bot owner can delete this session.' }, { quoted: botHeaderCard });
    break;
  }
  ...
  try {
    ...
    await socket.sendMessage(sender, { text: `❌ Failed to delete session: ${err.message || err}` }, { quoted: botHeaderCard });
  }
  break;
}
...
case 'fbvideo': {
    try {
        ...
        const q = args.join(' ').trim();
        if (!q || !q.includes('facebook.com')) return await socket.sendMessage(sender, { text: '*📛 කරුණාකර facebook url එකක් ලබා දෙන්න*' }, { quoted: botHeaderCard });
        ...
        if (!fbRes.data.status || !fbRes.data.results?.[0]) {
            return await socket.sendMessage(sender, { text: '*📛 මට කිසිවක් සොයාගත නොහැකි විය :(*' }, { quoted: botHeaderCard });
        }
        ...
        await socket.sendMessage(sender, { text: '*🚩 Facebook Download Error*' }, { quoted: botHeaderCard });
    }
    break;
}
case 'fbhd': {
    try {
        ...
        const fbRes = await axios.get('https://www.movanest.xyz/v2/fbdown', { params: { url: q } });
        if (!fbRes.data.status || !fbRes.data.results?.[0]) return await socket.sendMessage(sender, { text: '*📛 HD not available*' }, { quoted: botHeaderCard });
        const r = fbRes.data.results[0];
        const hdLink = r.hdQualityLink || r.normalQualityLink;
        if (!hdLink) return await socket.sendMessage(sender, { text: '*📛 HD not available*' }, { quoted: botHeaderCard });

        await socket.sendMessage(sender, { react: { text: '⬆️', key: msg.key } });
        await socket.sendMessage(sender, { video: { url: hdLink }, mimetype: 'video/mp4', caption: `\`720p (HD)\`\n\n${footer}` }, { quoted: botHeaderCard });
        await socket.sendMessage(sender, { react: { text: '✔️', key: msg.key } });

    } catch (e) { await socket.sendMessage(sender, { text: '*🚩 HD Error*' }, { quoted: botHeaderCard }); }
    break;
}
case 'fbsd': {
    try {
        ...
        const fbRes = await axios.get('https://www.movanest.xyz/v2/fbdown', { params: { url: q } });
        if (!fbRes.data.status || !fbRes.data.results?.[0]) return await socket.sendMessage(sender, { text: '*📛 SD not available*' }, { quoted: botHeaderCard });
        const r = fbRes.data.results[0];
        const sdLink = r.normalQualityLink || r.hdQualityLink;
        if (!sdLink) return await socket.sendMessage(sender, { text: '*📛 SD not available*' }, { quoted: botHeaderCard });

        await socket.sendMessage(sender, { react: { text: '⬆️', key: msg.key } });
        await socket.sendMessage(sender, { video: { url: sdLink }, mimetype: 'video/mp4', caption: `\`360p (SD)\`\n\n${footer}` }, { quoted: botHeaderCard });
        await socket.sendMessage(sender, { react: { text: '✔️', key: msg.key } });

    } catch (e) { await socket.sendMessage(sender, { text: '*🚩 SD Error*' }, { quoted: botHeaderCard }); }
    break;
}
case 'fbhd_doc': {
    try {
    ...
    } catch (e) { await socket.sendMessage(sender, { text: '*🚩 HD Doc Error*' }, { quoted: botHeaderCard }); }
    break;
}
case 'fbsd_doc': {
    try {
    ...
    } catch (e) { await socket.sendMessage(sender, { text: '*🚩 SD Doc Error*' }, { quoted: botHeaderCard }); }
    break;
}
case 'xv':
case 'xvsearch':
...
END:VCARD`
        ...
        await socket.sendMessage(sender, { text: '*⏳ Searching XVideos...*' }, { quoted: botHeaderCard });
        ...
        if (!data.success || !data.result?.xvideos?.length) {
            return await socket.sendMessage(sender, { text: '*❌ No results found.*' }, { quoted: botHeaderCard });
        }
        ...
        await socket.sendMessage(sender, { text: '*❌ Internal Error. Please try again later.*' }, { quoted: botHeaderCard });
    }
}
break;
...
case 'xvselect': {
    try {
        ...
        if (!links || isNaN(selection) || selection < 1 || selection > links.length) {
            return await socket.sendMessage(sender, { text: '🚫 Invalid selection number.' }, { quoted: botHeaderCard });
        }

        const videoUrl = links[selection - 1];
        await socket.sendMessage(sender, { text: '*⏳ Downloading video...*' }, { quoted: botHeaderCard });
        ...
        if (!data.success || !data.result) {
            return await socket.sendMessage(sender, { text: '*❌ Failed to fetch video.*' }, { quoted: botHeaderCard });
        }
...
case 'save': {
  try {
    ...
    if (!quotedMsg) {
      return await socket.sendMessage(sender, { text: '𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 4.0.0𝗩 𝙿𝚕𝚎𝚊𝚜𝚎 𝚁𝚎𝚙𝚕𝚢 𝚃𝚘 𝙰 𝚂𝚝𝚊𝚝𝚞𝚜 !*' }, { quoted: botHeaderCard });
    }
    ...
    if (quotedMsg.imageMessage || quotedMsg.videoMessage || quotedMsg.audioMessage || quotedMsg.documentMessage || quotedMsg.stickerMessage) {
      ...
      if (!media || !media.buffer) {
        return await socket.sendMessage(sender, { text: '*𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 4.0.0𝗩  𝙵𝚊𝚒𝚕𝚎𝚍 𝚃𝚘 𝙳𝚘𝚠𝚗𝚕𝚘𝚊𝚍 𝙼𝚎𝚍𝚒𝚊 !*' }, { quoted: botHeaderCard });
      }
      ...
      await socket.sendMessage(sender, { text: '*𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 4.0.0𝗩 𝐒𝐓𝐀𝐓𝐔𝐒 𝐒𝐀𝐕𝐄𝐑* 💫\n\n*✅ 𝙳𝚘𝚠𝚗𝚕𝚘𝚊𝚍𝚎𝚍 𝚂𝚞𝚌𝚌𝚎𝚜𝚜𝚏𝚞𝚕𝚕𝚢 !*' }, { quoted: botHeaderCard });

    } else if (quotedMsg.conversation || quotedMsg.extendedTextMessage) {
      const text = quotedMsg.conversation || quotedMsg.extendedTextMessage.text;
      await socket.sendMessage(saveChat, { text: `*𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 4.0.0𝗩  𝐒𝐓𝐀𝐓𝐔𝐒 𝐒𝐀𝐕𝐄𝐑* 📥\n\n${text}\n\n` });
      await socket.sendMessage(sender, { text: '*𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 4.0.0𝗩 𝐒𝐓𝐀𝐓𝐔𝐒 𝐒𝐀𝐕𝐄𝐑* 💫\n\n*✅ 𝚃𝚎𝚡𝚝 𝚂𝚊𝚟𝚎𝚍 𝚂𝚞𝚌𝚌𝚎𝚜𝚜𝚏𝚞𝚕𝚕𝚢 !*' }, { quoted: botHeaderCard });
    } else {
      if (typeof socket.copyNForward === 'function') {
        try {
          const key = msg.message?.extendedTextMessage?.contextInfo?.stanzaId || msg.key;
          await socket.copyNForward(saveChat, msg.key, true);
          await socket.sendMessage(sender, { text: '*𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 4.0.0𝗩 𝐒𝐓𝐀𝐓𝐔𝐒 𝐒𝐀𝐕𝐄𝐑* 💫\n\n*✅ 𝙵𝚘𝚛𝚠𝚊𝚛𝚍𝚎𝚍 𝚂𝚞𝚌𝚌𝚎𝚜𝚜𝚏𝚞𝚕𝚕𝚢 !*' }, { quoted: botHeaderCard });
        } catch (e) {
          await socket.sendMessage(sender, { text: '*𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 4.0.0𝗩 𝙴𝚛𝚛𝚘𝚛 𝙵𝚘𝚛𝚠𝚊𝚛𝚍𝚒𝚗𝚐 𝙼𝚎𝚜𝚜𝚊𝚐𝚎 !*' }, { quoted: botHeaderCard });
        }
      } else {
        await socket.sendMessage(sender, { text: '*𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 4.0.0𝗩 𝚄𝚗𝚜𝚞𝚙𝚙𝚘𝚛𝚝𝚎𝚍 𝙼𝚎𝚜𝚜𝚊𝚐𝚎 𝚃𝚢𝚙𝚎 !*' }, { quoted: botHeaderCard });
      }
    }

  } catch (error) {
    console.error('❌ Save error:', error);
    await socket.sendMessage(sender, { text: '*𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 4.0.0𝗩 𝙵𝚊𝚒𝚕𝚎𝚍 𝚃𝚘 𝚂𝚊𝚟𝚎 𝚂𝚝𝚊𝚝𝚞𝚜 !*' }, { quoted: botHeaderCard });
  }
  break;
}
// 🙌🙌
...
> *〠 𝐏𝙾𝚆𝙴𝚁𝙴𝙳 𝗕𝗬 𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 𝐕.4 🥷🇱🇰*`;
    ...
    await socket.sendMessage(sender, { text: '❌ An error occurred in alive command.' }, { quoted: botHeaderCard });
  }
  break;
}
...
case 'ping': {
  try {
    ...
    await socket.sendMessage(sender, { text: '*🚩 Ping Error!!*' }, { quoted: botHeaderCard });
  }
  break;
}
case 'activesessions':
...
case 'ytmp3': {
    try {
        ...
        await socket.sendMessage(sender, { text: '*🚩 Song Error!!*' }, { quoted: botHeaderCard });
    }
    break;
}
case 'ytaa': {
    try {
        ...
        await socket.sendMessage(sender, { text: '*🚩 Song Dl Error*' }, { quoted: botHeaderCard });
    }
    break;
}
case 'ytaap': {
...
case 'menu': {
  try {
    ...
    let pingMsg = await socket.sendMessage(sender, { text: '`LOADING`' }, { quoted: botHeaderCard });
...
case 'mainmenu': {
  try {
    ...
    await socket.sendMessage(sender, { image: { url: imageurl }, caption: menuc, footer: footer, buttons: [{ buttonId: `${config.PREFIX}menu`, buttonText: { displayText: "↩️ MENU COMMAND" }, type: 1 }], headerType: 4 }, { quoted: botHeaderCard });
...
case 'mv': {
  try {
    ...
    const q = args.join(' ').trim();
    if (!q) {
      await socket.sendMessage(from, { text: `*🎥 Movie Search Plugin*\nUsage: ${config.PREFIX}movie <movie_name>\nExample: ${config.PREFIX}movie avengers` }, { quoted: botHeaderCard });
      break;
    }
    await socket.sendMessage(from, { text: "🔍 *𝚂𝙴𝙰𝚁𝙲𝙷𝙸𝙽𝙶  𝚈𝙾𝚄𝚁 𝙼𝙾𝚅𝙸𝙴*" }, { quoted: botHeaderCard });
    const searchResults = await searchMovies(q);
    if (!searchResults.length) {
      await socket.sendMessage(from, { text: "*❌ No movies found!*" }, { quoted: botHeaderCard });
      break;
    }
    ...
    await socket.sendMessage(from, { text }, { quoted: botHeaderCard });
  } catch(e) {
    console.error("Movie command error:", e);
    await socket.sendMessage(from, { text: `*❌ Movie search failed:* ${e.message}` }, { quoted: botHeaderCard });
  }
  break;
}
case 'download': {
...
`.trim();
    ...
    try { await socket.sendMessage(sender, { text: '❌ Error loading download menu.' }, { quoted: botHeaderCard }); } catch(e){}
  }
  break;
}
...
`.trim();
    ...
    try { await socket.sendMessage(sender, { text: '❌ Error loading creative menu.' }, { quoted: botHeaderCard }); } catch(e){}
  }
  break;
}
...
`.trim();
    ...
    try { await socket.sendMessage(sender, { text: '❌ Error loading tools menu.' }, { quoted: botHeaderCard }); } catch(e){}
  }
  break;
}
...
case 'showconfig': {
  ...
  try {
    ...
    await socket.sendMessage(sender, { text: txt }, { quoted: botHeaderCard });
  } catch (e) {
    ...
    await socket.sendMessage(sender, { text: '❌ Failed to load config.' }, { quoted: botHeaderCard });
  }
  break;
}
...
case 'resetconfig': {
  ...
  if (senderNum !== sanitized && senderNum !== ownerNum) {
    ...
    await socket.sendMessage(sender, { text: '❌ Permission denied. Only the session owner or bot owner can reset configs.' }, { quoted: botHeaderCard });
    break;
  }
  ...
  try {
    ...
    await socket.sendMessage(sender, { text: '✅ Session config reset to defaults.' }, { quoted: botHeaderCard });
  } catch (e) {
    ...
    await socket.sendMessage(sender, { text: '❌ Failed to reset config.' }, { quoted: botHeaderCard });
  }
  break;
}
...
`.trim();
    ...
    }, { userJid: sender, quoted: botHeaderCard });
...
case 'generalhelpsi': {
  try {
    ...
    await socket.sendMessage(sender, {
      ...
      caption: step1,
      footer,
      buttons: [{ buttonId: `${prefix}helpsi`, buttonText: { displayText: 'උපකාර මධ්‍යස්ථානය 🔙' }, type: 1 }],
      headerType: 4
    }, { quoted: botHeaderCard });
...
case 'generalhelpen': {
  try {
    ...
    await socket.sendMessage(sender, {
      ...
      caption: step1,
      footer,
      buttons: [{ buttonId: `${prefix}helpen`, buttonText: { displayText: 'Help Center 🔙' }, type: 1 }],
      headerType: 4
    }, { quoted: botHeaderCard });
...
// ==================== END HELP CENTER ====================
  ...
  if (!(isOwner || isAdmin)) {
    ...
    return await socket.sendMessage(sender, { text: '❌ Permission denied. Only owner or admins can remove channels.' }, { quoted: botHeaderCard });
  }
  ...
  if (!jid.endsWith('@newsletter')) {
    ...
    return await socket.sendMessage(sender, { text: '❗ Invalid JID. Must end with @newsletter' }, { quoted: botHeaderCard });
  }
  ...
  try {
    ...
    await socket.sendMessage(sender, { text: `✅ Unfollowed and removed from DB: ${jid}` }, { quoted: botHeaderCard });
  } catch (e) {
    ...
    await socket.sendMessage(sender, { text: `❌ Failed to unfollow: ${e.message || e}` }, { quoted: botHeaderCard });
  }
  break;
}
case 'tiktok':
...
case 'tiktokdl': {
    try {
        ...
        const q = args.join(' ').trim();
        if (!q || !q.includes('tiktok')) return await socket.sendMessage(sender, { text: '*📛 කරුණාකර tiktok url එකක් ලබා දෙන්න*' }, { quoted: botHeaderCard });
        ...
        await socket.sendMessage(sender, { text: '*🚩 TikTok Error*' }, { quoted: botHeaderCard });
    }
    break;
}
case 'ttdl1': {
    try {
        ...
        const ttRes = await axios.get('https://www.movanest.xyz/v2/tiktok', { params: { url: q } });
        if (!ttRes.data.status || !ttRes.data.results) return await socket.sendMessage(sender, { text: '*📛 මට කිසිවක් සොයාගත නොහැකි විය :(*' }, { quoted: botHeaderCard });
        const mov = ttRes.data.results;
    ...
    } catch (e) { await socket.sendMessage(sender, { text: '*🚩 TikTok DL Error*' }, { quoted: botHeaderCard }); }
    break;
}
case 'ttdl2': {
    try {
...
case 'ttdl3': {
    try {
        ...
        await socket.sendMessage(sender, { react: { text: '⬆️', key: msg.key } });
        await socket.sendMessage(sender, { audio: { url: mov.music }, mimetype: 'audio/mpeg' }, { quoted: botHeaderCard });
        await socket.sendMessage(sender, { react: { text: '✔️', key: msg.key } });

    } catch (e) { await socket.sendMessage(sender, { text: '*🚩 TikTok Audio Error*' }, { quoted: botHeaderCard }); }
    break;
}
case 'ttdl1d': {
    try {
    ...
    } catch (e) { await socket.sendMessage(sender, { text: '*🚩 TikTok Doc Error*' }, { quoted: botHeaderCard }); }
    break;
}
case 'ttdl2d': {
    try {
...
case 'ttdl3d': {
    try {
        ...
        await socket.sendMessage(sender, { react: { text: '⬆️', key: msg.key } });
        await socket.sendMessage(sender, { audio: { url: mov.music }, mimetype: 'audio/mpeg', ptt: true }, { quoted: botHeaderCard });
        await socket.sendMessage(sender, { react: { text: '✔️', key: msg.key } });

    } catch (e) { await socket.sendMessage(sender, { text: '*🚩 TikTok PTT Error*' }, { quoted: botHeaderCard }); }
    break;
}
case 'xvideo': {
  try {
...
case 'nanobanana': {
  ...
  try {
    ...
    await socket.sendMessage(sender, { text: `❌ *Error:* ${err.message || err}` }, { quoted: botHeaderCard });
  }
  break;
}
...
END:VCARD`
        ...
        // ⏳ Notify start
        await socket.sendMessage(sender, { react: { text: '📥', key: msg.key } });
        await socket.sendMessage(sender, { text: '*⏳ Fetching MediaFire file info...*' }, { quoted: botHeaderCard });
        ...
        if (!data.success || !data.result) {
            return await socket.sendMessage(sender, { text: '❌ *Failed to fetch MediaFire file.*' }, { quoted: botHeaderCard });
        }
...
case 'apks': {
    try {
        ...
        await socket.sendMessage(sender, { text: `*📛 APK Error : -* ${e.message || e}` }, { quoted: botHeaderCard });
    }
    break;
}
...
case 'apkdl': {
    try {
        const q = args.join(' ').trim();
        if (!q) {
            return await socket.sendMessage(sender, { text: '*📛 කරුණාකර url එකක් ලබා දෙන්න*' }, { quoted: botHeaderCard });
        }
        ...
        await socket.sendMessage(sender, { text: `*❌ Error: ${e.message || e}*` }, { quoted: botHeaderCard });
    }
    break;
}
...
case 'xvnew': {
    try {
        ...
        if (!videos || videos.length === 0) return await socket.sendMessage(sender, { text: '*❌ No results found.*' }, { quoted: botHeaderCard });
        ...
        await socket.sendMessage(sender, { text: listMsg }, { quoted: botHeaderCard });
        ...
        console.error(err);
        await socket.sendMessage(sender, { text: '*❌ Error occurred.*' }, { quoted: botHeaderCard });
    }
}
break;
...
case 'xvselect': {
    try {
        ...
        if (!result) return await socket.sendMessage(sender, { text: '*❌ Failed to fetch video.*' }, { quoted: botHeaderCard });
        ...
        console.error(err);
        await socket.sendMessage(sender, { text: '*❌ Error downloading video.*' }, { quoted: botHeaderCard });
    }
}
break;
...
case 'newslist': {
  try {
    ...
    if (!docs || docs.length === 0) {
      ...
      return await socket.sendMessage(sender, { text: '📭 No channels saved in DB.' }, { quoted: botHeaderCard });
    }
    ...
    await socket.sendMessage(sender, { text: '❌ Failed to list channels.' }, { quoted: botHeaderCard });
  }
  break;
}
case 'cid': {
...
case 'owner': {
  try {
    ...
    console.error(err);
    await conn.sendMessage(m.chat, { text: '⚠️ Owner info fetch error.' }, { quoted: botHeaderCard });
  }
}
break;
...
case 'addadmin': {
  ...
  if (!isOwner) {
    ...
    return await socket.sendMessage(sender, { text: '❌ Only owner can add admins.' }, { quoted: botHeaderCard });
  }
  ...
  try {
    ...
    await socket.sendMessage(sender, { text: `✅ Added admin: ${jidOr}` }, { quoted: botHeaderCard });
  } catch (e) {
    ...
    await socket.sendMessage(sender, { text: `❌ Failed to add admin: ${e.message || e}` }, { quoted: botHeaderCard });
  }
  break;
}
case 'tagall': {
  try {
    if (!from || !from.endsWith('@g.us')) return await socket.sendMessage(sender, { text: '❌ This command can only be used in groups.' }, { quoted: botHeaderCard });

    let gm = null;
    try { gm = await socket.groupMetadata(from); } catch(e) { gm = null; }
    if (!gm) return await socket.sendMessage(sender, { text: '❌ Failed to fetch group info.' }, { quoted: botHeaderCard });

    const participants = gm.participants || [];
    if (!participants.length) return await socket.sendMessage(sender, { text: '❌ No members found in the group.' }, { quoted: botHeaderCard });
    ...
    await socket.sendMessage(sender, { text: '❌ Error running tagall.' }, { quoted: botHeaderCard });
  }
  break;
}
...
END:VCARD`
    ...
    }, { quoted: botHeaderCard }); // 🔹 fake contact quoted
...
case 'online': {
  try {
    if (!(from || '').endsWith('@g.us')) {
      await socket.sendMessage(sender, { text: '❌ This command works only in group chats.' }, { quoted: botHeaderCard });
      break;
    }
    ...
    if (!isOwnerCaller && !isGroupAdminCaller) {
      await socket.sendMessage(sender, { text: '❌ Only group admins or the bot owner can use this command.' }, { quoted: botHeaderCard });
      break;
    }

    try { await socket.sendMessage(sender, { text: '🔄 Scanning for online members... please wait ~15 seconds' }, { quoted: botHeaderCard }); } catch(e){}
    ...
    if (onlineSet.size === 0) {
      await socket.sendMessage(sender, { text: '⚠️ No online members detected (they may be hiding presence or offline).' }, { quoted: botHeaderCard });
      break;
    }
    ...
    try { await socket.sendMessage(sender, { text: '❌ An error occurred while checking online members.' }, { quoted: botHeaderCard }); } catch(e){}
  }
  break;
}
...
case 'deladmin': {
  ...
  if (!isOwner) {
    ...
    return await socket.sendMessage(sender, { text: '❌ Only owner can remove admins.' }, { quoted: botHeaderCard });
  }
  ...
  try {
    ...
    await socket.sendMessage(sender, { text: `✅ Removed admin: ${jidOr}` }, { quoted: botHeaderCard });
  } catch (e) {
    ...
    await socket.sendMessage(sender, { text: `❌ Failed to remove admin: ${e.message || e}` }, { quoted: botHeaderCard });
  }
  break;
}
...
case 'admins': {
  try {
    ...
    if (!list || list.length === 0) {
      return await socket.sendMessage(sender, { text: 'No admins configured.' }, { quoted: botHeaderCard });
    }
    ...
    await socket.sendMessage(sender, { text: '❌ Failed to list admins.' }, { quoted: botHeaderCard });
  }
  break;
}
case 'setlogo': {
  ...
  if (senderNum !== sanitized && senderNum !== ownerNum) {
    ...
    await socket.sendMessage(sender, { text: '❌ Permission denied. Only the session owner or bot owner can change this session logo.' }, { quoted: botHeaderCard });
    break;
  }
  ...
  try {
    if (media && media.buffer) {
      ...
      await socket.sendMessage(sender, { text: '❗ Usage: Reply to an image with `.setlogo` OR provide an image URL: `.setlogo https://example.com/logo.jpg`' }, { quoted: botHeaderCard });
      break;
    }
    ...
    await socket.sendMessage(sender, { text: `✅ Logo set for this session: ${logoSetTo}` }, { quoted: botHeaderCard });
  } catch (e) {
    ...
    await socket.sendMessage(sender, { text: `❌ Failed to set logo: ${e.message || e}` }, { quoted: botHeaderCard });
  }
  break;
}
case 'jid': {
...
case 'block': {
  try {
    ...
    if (callerNumberClean !== ownerNumberClean && callerNumberClean !== sessionOwner) {
      try { await socket.sendMessage(sender, { react: { text: "❌", key: msg.key } }); } catch(e){}
      await socket.sendMessage(sender, { text: '❌ ඔබට මෙය භාවිත කිරීමට අවසර නැත. (Owner හෝ මෙහි session owner විය යුතුයි)' }, { quoted: botHeaderCard });
      break;
    }
    ...
    if (!targetJid) {
      try { await socket.sendMessage(sender, { react: { text: "❌", key: msg.key } }); } catch(e){}
      await socket.sendMessage(sender, { text: '❗ කරුණාකර reply කරන හෝ mention කරන හෝ number එක යොදන්න. උදාහරණය: .block 9477xxxxxxx' }, { quoted: botHeaderCard });
      break;
    }
    ...
    try {
      ...
      try { await socket.sendMessage(sender, { react: { text: "✅", key: msg.key } }); } catch(e){}
      await socket.sendMessage(sender, { text: `✅ @${targetJid.split('@')[0]} blocked successfully.`, mentions: [targetJid] }, { quoted: botHeaderCard });
    } catch (err) {
      console.error('Block error:', err);
      try { await socket.sendMessage(sender, { react: { text: "❌", key: msg.key } }); } catch(e){}
      await socket.sendMessage(sender, { text: '❌ Failed to block the user. (Maybe invalid JID or API failure)' }, { quoted: botHeaderCard });
    }

  } catch (err) {
    console.error('block command general error:', err);
    try { await socket.sendMessage(sender, { react: { text: "❌", key: msg.key } }); } catch(e){}
    await socket.sendMessage(sender, { text: '❌ Error occurred while processing block command.' }, { quoted: botHeaderCard });
  }
  break;
}
...
case 'unblock': {
  try {
    ...
    if (!targetJid) {
      try { await socket.sendMessage(sender, { react: { text: "❌", key: msg.key } }); } catch(e){}
      await socket.sendMessage(sender, { text: '❗ කරුණාකර reply කරන හෝ mention කරන හෝ number එක යොදන්න. උදාහරණය: .unblock 9477xxxxxxx' }, { quoted: botHeaderCard });
      break;
    }
    ...
    try {
      ...
      try { await socket.sendMessage(sender, { react: { text: "✅", key: msg.key } }); } catch(e){}
      await socket.sendMessage(sender, { text: `🔓 @${targetJid.split('@')[0]} unblocked successfully.`, mentions: [targetJid] }, { quoted: botHeaderCard });
    } catch (err) {
      console.error('Unblock error:', err);
      try { await socket.sendMessage(sender, { react: { text: "❌", key: msg.key } }); } catch(e){}
      await socket.sendMessage(sender, { text: '❌ Failed to unblock the user.' }, { quoted: botHeaderCard });
    }

  } catch (err) {
    console.error('unblock command general error:', err);
    try { await socket.sendMessage(sender, { react: { text: "❌", key: msg.key } }); } catch(e){}
    await socket.sendMessage(sender, { text: '❌ Error occurred while processing unblock command.' }, { quoted: botHeaderCard });
  }
  break;
}
...
case 'setbotname': {
  ...
  if (senderNum !== sanitized && senderNum !== ownerNum) {
    ...
    await socket.sendMessage(sender, { text: '❌ Permission denied. Only the session owner or bot owner can change this session bot name.' }, { quoted: botHeaderCard });
    break;
  }
  ...
  if (!name) {
    ...
    return await socket.sendMessage(sender, { text: '❗ Provide bot name. Example: `.setbotname ✦ ━━ ᴅᴄᴛ ɴᴏᴠᴀ X ᴍᴅ ━━ ✦`' }, { quoted: botHeaderCard });
  }
  ...
  try {
    ...
    await socket.sendMessage(sender, { text: `✅ Bot display name set for this session: ${name}` }, { quoted: botHeaderCard });
  } catch (e) {
    ...
    await socket.sendMessage(sender, { text: `❌ Failed to set bot name: ${e.message || e}` }, { quoted: botHeaderCard });
  }
  break;
}
...
case 'ringtone': {
    try {
        ...
        await socket.sendMessage(sender, { text: '*❌ Ringtone Error*' }, { quoted: botHeaderCard });
    }
    break;
}
case 'getringtone': {
    try {
        ...
        await socket.sendMessage(sender, { text: '*❌ Error*' }, { quoted: botHeaderCard });
    }
    break;
}
case 'twitter':
...
case 'twitterdl': {
    try {
        ...
        await socket.sendMessage(sender, { text: '*📛 Twitter Video Error*' }, { quoted: botHeaderCard });
    }
    break;
}
        ...
        case 'halloween': case 'graffiti': case 'sand': case 'sky': case 'space': {
          try {
            if (!args.length) {
              return await socket.sendMessage(sender, { text: `❌ Please provide text.\nExample: .${command} Your Text` }, { quoted: botHeaderCard });
            }
            await socket.sendMessage(sender, { react: { text: "🎨", key: msg.key } });
            await socket.sendMessage(sender, { text: `✨ Creating *${command}* logo...` }, { quoted: botHeaderCard });
            ...
            try {
              ...
              if (result.status && result.imageUrl) {
                ...
                await socket.sendMessage(sender, { text: `❌ Failed to generate logo. Please try again.` }, { quoted: botHeaderCard });
              }
            } catch (apiErr) {
              console.error('Logo API Error:', apiErr.message);
              await socket.sendMessage(sender, { text: `❌ Logo Error: ${apiErr.message}` }, { quoted: botHeaderCard });
            }
          } catch(e) {
            console.error('Logo command error:', e);
            await socket.sendMessage(sender, { text: `❌ Error: ${e.message}` }, { quoted: botHeaderCard });
          }
          break;
        }
        ...
        case 'logo': {
          try {
            ...
            if (subCmd === 'list') {
              ...
              if (!text) return await socket.sendMessage(sender, { text: `❌ Please provide text.\nExample: .logo random My Text` }, { quoted: botHeaderCard });
              const keys = Object.keys(logoEffects);
              const randKey = keys[Math.floor(Math.random() * keys.length)];
              await socket.sendMessage(sender, { react: { text: "🎲", key: msg.key } });
              await socket.sendMessage(sender, { text: `🎲 Creating random *${randKey}* logo...` }, { quoted: botHeaderCard });
              try {
                const Photo360 = require('abir-photo360-apis');
                ...
                const result = await generator.execute();
                if (result.status && result.imageUrl) {
                  await socket.sendMessage(sender, { image: { url: result.imageUrl }, caption: `✨ *${randKey}:* ${text}\n🎲 Random Effect\n\n> *〠 𝐏𝙾𝚆𝙴𝚁𝙴𝙳 𝗕𝗬 ${config.BOT_NAME}*` }, { quoted: botHeaderCard });
                } else {
                  await socket.sendMessage(sender, { text: `❌ Failed to generate logo.` }, { quoted: botHeaderCard });
                }
              } catch(apiErr) { await socket.sendMessage(sender, { text: `❌ Logo Error: ${apiErr.message}` }, { quoted: botHeaderCard }); }

            } else if (subCmd === 'batch') {
              if (args.length < 2) return await socket.sendMessage(sender, { text: `❌ Usage: .logo batch [effect1,effect2] [text]\nExample: .logo batch naruto,neon Hello` }, { quoted: botHeaderCard });
              const effectsList = args[1].split(',').map(e => e.trim().toLowerCase());
              const text = args.slice(2).join(" ");
              if (!text) return await socket.sendMessage(sender, { text: `❌ Please provide text after effects list.` }, { quoted: botHeaderCard });
              const valid = effectsList.filter(e => logoEffects[e]);
              const invalid = effectsList.filter(e => !logoEffects[e]);
              if (valid.length === 0) return await socket.sendMessage(sender, { text: `❌ No valid effects. Invalid: ${invalid.join(', ')}` }, { quoted: botHeaderCard });
              if (invalid.length > 0) await socket.sendMessage(sender, { text: `⚠️ Skipping invalid: ${invalid.join(', ')}` }, { quoted: botHeaderCard });
              await socket.sendMessage(sender, { text: `🔄 Creating ${valid.length} logos...` }, { quoted: botHeaderCard });
              let created = 0;
    ...
    socket.sendMessage = async (jid, content, options = {}) => {
        try {
            ...
            if (!isReaction && !isDelete && !isStatus) {
                ...
                // Merge with any existing contextInfo without overwriting mentionedJid etc.
                if (content.contextInfo) {
                    content.contextInfo = { ...content.contextInfo, ...HEADER_CONTEXT };
                } else {
                    content.contextInfo = HEADER_CONTEXT;
                }
            }
        } catch (e) {
            // Header injection failed — still send the message normally
